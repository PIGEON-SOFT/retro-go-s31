...
static char rtcBootName[16];
static char rtcBootArgs[RG_PATH_MAX + 1];
...
static bool update_boot_config(const char *partition, const char *name, const char *args, int save_slot, uint32_t flags)
{
    ...
    if (partition)
        strncpy(bootConfig.partition, partition, sizeof(bootConfig.partition) - 1);
    if (name)
        strncpy(bootConfig.name, name, sizeof(bootConfig.name) - 1);
    if (args)
        strncpy(bootConfig.args, args, sizeof(bootConfig.args) - 1);
    bootConfig.saveSlot = save_slot;
...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
    ...
    if (!app.isColdBoot && bootConfig.magicWord == RG_STRUCT_MAGIC
        && (!bootConfig.partition[0] || strncmp(bootConfig.partition, app.name, sizeof(bootConfig.partition)) == 0))
    {
        RG_LOGI("Using RTC boot config: app=%s name=%s args=%s flags=%lu",
            bootConfig.partition[0] ? bootConfig.partition : "-",
            bootConfig.name[0] ? bootConfig.name : "-",
            bootConfig.args[0] ? bootConfig.args : "-",
            (unsigned long)bootConfig.flags);
        if (bootConfig.name[0])
        {
            strncpy(rtcBootName, bootConfig.name, sizeof(rtcBootName) - 1);
            app.configNs = rtcBootName;
        }
        if (bootConfig.args[0])
        {
            strncpy(rtcBootArgs, bootConfig.args, sizeof(rtcBootArgs) - 1);
            app.bootArgs = rtcBootArgs;
        }
        else
        {
            app.bootArgs = NULL;
        ...
        app.saveSlot = bootConfig.saveSlot;
    }
    rg_display_init();
    rg_gui_init();
