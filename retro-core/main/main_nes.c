...
void nes_main(void)
{
    ...
    while (true)
    {
        ...
        if (rg_netplay_mode() != NETPLAY_MODE_NONE)
        {
            rg_netplay_sync(&buttons, &remote_buttons, sizeof(buttons));
        }
#endif
...
#ifdef RG_ENABLE_NETPLAY
        if (rg_netplay_mode() == NETPLAY_MODE_GUEST)
        {
            input_update(0, remote_buttons);
