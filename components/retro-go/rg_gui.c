...
static size_t get_glyph(uint32_t *output, const rg_font_t *font, int points, int c)
{
    ...
    if (glyph && glyph->code == c) // Glyph found
    {
        ...
        if (output)
        {
            uint32_t temp_bitmap[64];
            uint32_t *target = output;
            int target_height = points;

            if (points != font->height)
            {
                target = temp_bitmap;
                target_height = RG_MIN((int)RG_COUNT(temp_bitmap), (int)font->height);
            }

            memset(target, 0, target_height * sizeof(uint32_t));
            int ch = 0, mask = 0x80;
            for (int y = 0; y < height; y++)
            {
                uint32_t row = 0;
                ...
                int out_y = yOffset + y;
                if (out_y >= 0 && out_y < target_height)
                    target[out_y] = row;
            }
            // Vertical stretching
            if (points != font->height)
            {
                ...
                memset(output, 0, points * sizeof(uint32_t));
                for (int y = points - 1; y >= 0; y--)
                    output[y] = target[RG_MIN((int)(y / scale), target_height - 1)];
            }
        }
    ...
    // }
    else if (font != &font_STHeitiMedium14 && c >= 0x2E80 && c <= 0xFFFF)
    {
        return get_glyph(output, &font_STHeitiMedium14, points, c);
    }
...
static rg_gui_event_t show_fps_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    bool enabled = rg_display_get_fps_overlay();
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_display_set_fps_overlay(enabled = !enabled);
...
static int wifi_find_available_slot(void)
{
    rg_wifi_config_t config = {0};
...
static bool wifi_save_and_connect(int slot, const char *ssid, const char *password, int channel)
{
    rg_wifi_config_t config = {0};
    ...
    if (!rg_network_wifi_write_config(slot, &config))
    {
        rg_gui_alert(_("Error"), _("Failed to save network configuration"));
        return false;
    }
    ...
    rg_network_t info = rg_network_get_info();
    if (info.state == RG_NETWORK_CONNECTED)
        rg_gui_alert(_("Success"), info.ip_addr[0] ? info.ip_addr : _("Connected"));
    else
        rg_gui_alert(_("Connection failed!"), _("Not connected"));

    return info.state == RG_NETWORK_CONNECTED;
}

static rg_gui_event_t wifi_scan_networks_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event != RG_DIALOG_ENTER)
        return RG_DIALOG_VOID;

    enum { WIFI_SCAN_MAX = 16 };
    rg_wifi_scan_result_t results[WIFI_SCAN_MAX] = {0};
    size_t count = 0;

    rg_gui_draw_message(_("Scanning..."));
    if (!rg_network_wifi_scan(results, WIFI_SCAN_MAX, &count))
    {
        rg_gui_alert(_("Error"), _("Wi-Fi scan failed"));
        return RG_DIALOG_REDRAW;
    }
    if (count == 0)
    {
        rg_gui_alert(_("Wi-Fi scan"), _("No networks found"));
        return RG_DIALOG_REDRAW;
    }

    rg_gui_option_t scan_options[WIFI_SCAN_MAX + 1] = {0};
    char *labels[WIFI_SCAN_MAX] = {0};

    for (size_t i = 0; i < count; ++i)
    {
        char label[80];
        snprintf(label, sizeof(label), "%.28s  %ddBm %s",
                 results[i].ssid, results[i].rssi, results[i].auth ? "*" : "open");
        labels[i] = strdup(label);
        scan_options[i] = (rg_gui_option_t){i + 1, labels[i], NULL, RG_DIALOG_FLAG_NORMAL, NULL};
    }
    scan_options[count] = (rg_gui_option_t)RG_DIALOG_END;

    int selected = rg_gui_dialog(_("Wi-Fi scan"), scan_options, 0);
    for (size_t i = 0; i < count; ++i)
        free(labels[i]);

    if (selected == RG_DIALOG_CANCELLED || selected < 1 || (size_t)selected > count)
        return RG_DIALOG_REDRAW;

    rg_wifi_scan_result_t *network = &results[selected - 1];
    int slot = wifi_find_available_slot();

    if (slot == 0)
    {
        rg_wifi_config_t existing = {0};
        if (rg_network_wifi_read_config(0, &existing) &&
            !rg_gui_confirm(_("Overwrite slot 0?"), _("All Wi-Fi slots are full.\nReplace slot 0?"), true))
        {
            return RG_DIALOG_REDRAW;
        }
    }
    ...
    char message[96];
    snprintf(message, sizeof(message), "SSID:\n%s\n\n%s",
             network->ssid, _("Enter password (leave empty for open network):"));
    char *password = rg_gui_input_str(_("Wi-Fi Password"), message, "");
    if (!password)
        return RG_DIALOG_REDRAW;

    wifi_save_and_connect(slot, network->ssid, password, network->channel);
    free(password);
    return RG_DIALOG_REDRAW;
}
...
static rg_gui_event_t wifi_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        const rg_gui_option_t options[] = {
            ...
            {0x00, _("Scan networks"),      NULL, RG_DIALOG_FLAG_NORMAL,  &wifi_scan_networks_cb },
...
void rg_gui_options_menu(void)
{
    ...
    const rg_gui_option_t game_options[] = {
        ...
        {0, _("Show FPS"),      "-", RG_DIALOG_FLAG_NORMAL, &show_fps_cb},
...
void rg_gui_game_menu(void)
{
    ...
    #ifdef RG_ENABLE_NETPLAY
        case 5000: if (rg_netplay_quick_start()) rg_emu_reset(true); break;
    #endif
