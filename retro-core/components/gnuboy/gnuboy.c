...
static byte failed_rom_bank[BANK_SIZE];
static bool failed_rom_bank_ready = false;
static bool rom_bank_io_error = false;
...
bool gnuboy_load_bank(int bank)
{
	...
	{
		for (int tries = 0; tries < cart.romsize && !cart.rombanks[bank]; tries++)
		{
			int i = rand() % cart.romsize;
			if (cart.rombanks[i] && cart.rombanks[i] != failed_rom_bank)
			{
		...
		if (!cart.rombanks[bank])
		{
			MESSAGE_ERROR("ROM bank allocation failed\n");
			if (!failed_rom_bank_ready)
			{
				memset(failed_rom_bank, 0xFF, sizeof(failed_rom_bank));
				failed_rom_bank_ready = true;
			}
			cart.rombanks[bank] = failed_rom_bank;
			rom_bank_io_error = true;
			return false;
		}
	}
	...
	for (int attempt = 0; attempt < 3; attempt++)
	{
		...
		MESSAGE_WARN("ROM bank loading failed, bank=%d attempt=%d\n", bank, attempt + 1);
	}
...
int gnuboy_load_rom_file(const char *file)
{
	...
	// Keep startup memory modest. Banks are loaded on demand after boot.

	int preload = cart.romsize < 2 ? cart.romsize : 2;
	...
	if (cart.romsize > 64 && (strncmp(cart.name, "RAYMAN", 6) == 0 || strncmp(cart.name, "NONAME", 6) == 0))
	{
		...
		preload = cart.romsize > 40 && (cart.romsize - 40) < preload ? (cart.romsize - 40) : preload;
	}

	MESSAGE_INFO("Preloading the first %d banks\n", preload);
	for (int i = 0; i < preload; i++)
	{
		if (!gnuboy_load_bank(i))
			return -4;
	}
...
bool gnuboy_rom_bank_error(void)
{
	return rom_bank_io_error;
}
...
void gnuboy_free_rom(void)
{
	...
	if (cart.romFile && cart.rombanks)
	{
		...
		{
			if (cart.rombanks[i] != failed_rom_bank)
