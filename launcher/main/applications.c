...
static void app_add_file(retro_app_t *app, const char *folder, const char *name, uint8_t type)
{
...
static int scan_folder_cb(const rg_scandir_t *entry, void *arg)
{
    ...
    app_add_file(app, entry->dirname, entry->basename, type);
...
void applications_init(void)
{
    ...
    application("Game Boy", "gb", "gb gbc zip", "retro-core", 0);
    application("Game Boy Color", "gbc", "gbc gb zip", "retro-core", 0);
    application("Game Boy Advance", "gba", "gba zip", "gbsp", 0);
    application("Game & Watch", "gw", "gw mgw", "retro-core", 0);
