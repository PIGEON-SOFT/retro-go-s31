...
bool rg_network_wifi_scan(rg_wifi_scan_result_t *results, size_t max_results, size_t *out_count)
{
#ifdef RG_ENABLE_NETWORKING
    if (!results || max_results == 0 || network_state == RG_NETWORK_DISABLED)
        return false;

    esp_err_t err;
    rg_wifi_config_t saved_config = wifi_config;
    bool restore_wifi = rg_settings_get_boolean(NS_WIFI, SETTING_WIFI_ENABLE, false) && saved_config.ssid[0];
    wifi_ap_record_t *records = calloc(max_results, sizeof(*records));
    if (!records)
        return false;
    ...
    TRY(esp_wifi_set_mode(WIFI_MODE_STA));
    TRY(esp_wifi_start());
    TRY(esp_wifi_scan_start(&scan_config, true));

    uint16_t ap_count = 0;
    TRY(esp_wifi_scan_get_ap_num(&ap_count));
    ap_count = RG_MIN(ap_count, (uint16_t)max_results);
    TRY(esp_wifi_scan_get_ap_records(&ap_count, records));

    size_t count = 0;
    for (uint16_t i = 0; i < ap_count && count < max_results; ++i)
    {
        if (!records[i].ssid[0])
            continue;
        ...
        strncpy(results[count].ssid, (const char *)records[i].ssid, sizeof(results[count].ssid) - 1);
        results[count].channel = records[i].primary;
        results[count].rssi = records[i].rssi;
        results[count].auth = records[i].authmode != WIFI_AUTH_OPEN;
        count++;
    }
    ...
    esp_wifi_stop();
    wifi_scanning = false;
    rg_network_wifi_set_config(&saved_config);
    if (restore_wifi)
