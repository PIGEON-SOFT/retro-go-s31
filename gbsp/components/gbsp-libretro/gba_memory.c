...
static const u8 *gamepak_memory_large = NULL;
static u32 gamepak_memory_size = 0;
static u8 *gamepak_memory_owned = NULL;
...
u8 *load_gamepak_page(u32 physical_index)
{
  ...
  if (gamepak_memory_large)
  {
    u32 offset = physical_index * (32 * 1024);
    u32 bytes = 0;
    if (offset < gamepak_memory_size)
      bytes = gamepak_memory_size - offset;
    if (bytes > (32 * 1024))
      bytes = 32 * 1024;
    memcpy(swap_location, gamepak_memory_large + offset, bytes);
    if (bytes < (32 * 1024))
      memset(swap_location + bytes, 0xFF, (32 * 1024) - bytes);
  }
  else
  {
...
void memory_term(void)
{
  ...
  if (gamepak_memory_owned)
  {
    free(gamepak_memory_owned);
    gamepak_memory_owned = NULL;
  }
  gamepak_memory_large = NULL;
  gamepak_memory_size = 0;
...
static bool gamepak_path_has_extension(const char *name, const char *extension)
{
...
static void load_gamepak_bytes(const u8 *data, u32 data_size)
{
  unsigned i, j;
  gamepak_memory_large = data;
  gamepak_memory_size = data_size;

  gamepak_size = (data_size + 0x7FFF) & ~0x7FFF;
  ...
  for (i = 0; i < ldblks; i++)
  {
    u32 offset = i * gamepak_buffer_blocksize;
    u32 bytes = data_size > offset ? data_size - offset : 0;
    if (bytes > gamepak_buffer_blocksize)
      bytes = gamepak_buffer_blocksize;
    memcpy(gamepak_buffers[i], data + offset, bytes);
    if (bytes < gamepak_buffer_blocksize)
      memset(gamepak_buffers[i] + bytes, 0xFF, gamepak_buffer_blocksize - bytes);
...
static s32 load_gamepak_raw(const char *name)
{
  ...
  if (gamepak_path_has_extension(name, "zip"))
  {
    void *data = NULL;
    size_t data_len = 0;
    if (rg_storage_unzip_file(name, "gba", &data, &data_len, 0))
    {
      gamepak_memory_owned = data;
      load_gamepak_bytes(gamepak_memory_owned, data_len);
      return 0;
    }
    return -1;
  }
