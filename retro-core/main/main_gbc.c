...
void gbc_main(void)
{
    ...
    void *audioBuffer = malloc(AUDIO_BUFFER_LENGTH * 4);
    if (!audioBuffer)
    {
        rg_gui_alert("ROM load failed", "Not enough memory for audio buffer.");
        rg_system_switch_app(RG_APP_LAUNCHER, NULL, NULL, 0, 0);
        return;
    }
    gnuboy_set_soundbuffer(audioBuffer, AUDIO_BUFFER_LENGTH);
    ...
    if (rg_extension_match(app->romPath, "zip"))
    {
        ...
        {
            rg_gui_alert("ROM load failed", "Unable to unzip this ROM.");
            rg_system_switch_app(RG_APP_LAUNCHER, NULL, NULL, 0, 0);
            return;
        }
        if (gnuboy_load_rom(data, size) < 0)
        {
            rg_gui_alert("ROM load failed", "This GB/GBC ROM could not be loaded.");
            rg_system_switch_app(RG_APP_LAUNCHER, NULL, NULL, 0, 0);
            return;
        }
    }
    ...
    while (true)
    {
        ...
        if (gnuboy_rom_bank_error())
        {
            rg_gui_alert("ROM read failed", "SD card read failed while loading this game.");
            rg_system_switch_app(RG_APP_LAUNCHER, NULL, NULL, 0, 0);
            return;
        }
